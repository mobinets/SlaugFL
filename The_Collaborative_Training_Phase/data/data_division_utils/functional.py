...
if __name__ == '__main__':
    ...
    import torch
    import numpy as np
    dataset_train = datasets.CIFAR10('/home/demo2/FL_data/cifar-10/cifar-10-batches-py', train=True, download=False, transform=None)
    targets = dataset_train.targets
    client_idx=hetero_dir_partition(targets,20,10,0.05)
    tensor = np.zeros((20,10))
    np.set_printoptions(suppress=True)
    for cli in range(len(client_idx)):
        for index in client_idx[cli]:
            tensor[cli][targets[index]] +=1
    print(tensor)
